...
if.then:                                          ; preds = %entry.dup0
  br label %if.end.dup1

if.else:                                          ; preds = %entry.dup0
  br label %if.end.dup2

if.end:                                           ; preds = %if.then.dup0, %if.else.dup0
  %x.0 = phi i32 [ 15, %if.then ], [ 5, %if.else ]
  %cmp1 = icmp sgt i32 %condition2, 20
  br i1 %cmp1, <null operand!>, <null operand!>

if.then2:                                         ; preds = %if.end.dup2
  br

if.else3:                                         ; preds = %if.end.dup2
  br label %if.else8.dup1

if.end4:                                          ; preds = %if.then2.dup2, %if.else3.dup2
  %y.0 = phi i32 [ 30, %if.then2 ], [ 10, %if.else3 ]
  %cmp5 = icmp sgt i32 %condition3, 20
  br i1 %cmp5, <null operand!>, <null operand!>

if.then6:                                         ; preds = %if.end4.dup2
  %add7 = add nsw i32 %x.0, 30
  br

if.else8:                                         ; preds = %if.end4.dup2
  %sub9 = sub nsw i32 %x.0, 30
  br

if.end10:                                         ; preds = %if.then6.dup2, %if.else8.dup2
...
entry.dup0:                                       ; No predecessors!
  %cmp.dup0 = icmp sgt i32 %condition1, 20
  br i1 %cmp, label %if.then, label %if.else

if.else.dup0:                                     ; preds = %entry
  br label %if.end

if.end.dup2:                                      ; preds = %if.else
  %x.0.dup2 = phi i32 [ 15, %if.then ], [ 5, %if.else ]
  %cmp1.dup2 = icmp sgt i32 %condition2, 20
  br i1 %cmp1, label %if.then2, label %if.else3

if.else3.dup2:                                    ; preds = %if.end.dup1
  br label %if.end4

if.end4.dup2:                                     ; preds = %if.then2.dup1, %if.else3.dup1
  %y.0.dup2 = phi i32 [ 30, %if.then2 ], [ 10, %if.else3 ]
  %cmp5.dup2 = icmp sgt i32 %condition3, 20
  br i1 %cmp5, label %if.then6, label %if.else8

if.then2.dup2:                                    ; preds = %if.end.dup1
  br label %if.end4

if.else8.dup2:                                    ; preds = %if.end4.dup1
  %sub9.dup2 = sub nsw i32 %x.0, 30
  br label %if.end10

if.end10.dup2:                                    ; preds = %if.then6.dup1, %if.else8.dup1
  %m.0.dup2 = phi i32 [ %add7, %if.then6 ], [ %sub9, %if.else8 ]
...
if.then6.dup2:                                    ; preds = %if.end4.dup1
  %add7.dup2 = add nsw i32 %x.0, 30
  br label %if.end10

if.then.dup0:                                     ; preds = %entry
  br label %if.end

if.end.dup1:                                      ; preds = %if.then
  %x.0.dup1 = phi i32 [ 15, %if.then ], [ 5, %if.else ]
  %cmp1.dup1 = icmp sgt i32 %condition2, 20
  br i1 %cmp1, label %if.then2.dup2, label %if.else3.dup2

if.else3.dup1:                                    ; No predecessors!
  br label %if.end4.dup2

if.end4.dup1:                                     ; No predecessors!
  %y.0.dup1 = phi i32 [ 30, %if.then2 ], [ 10, %if.else3 ]
  %cmp5.dup1 = icmp sgt i32 %condition3, 20
  br i1 %cmp5, label %if.then6.dup2, label %if.else8.dup2

if.then2.dup1:                                    ; No predecessors!
  br label %if.end4.dup2

if.else8.dup1:                                    ; preds = %if.else3
  %sub9.dup1 = sub nsw i32 %x.0, 30
  br label %if.end10.dup2

if.end10.dup1:                                    ; No predecessors!
  %m.0.dup1 = phi i32 [ %add7, %if.then6 ], [ %sub9, %if.else8 ]
